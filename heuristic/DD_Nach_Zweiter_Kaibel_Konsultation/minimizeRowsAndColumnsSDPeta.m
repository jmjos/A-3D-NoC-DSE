%% WARNINNG
MACHT NICHT DAS WAS WIR WOLLEN, WEIL ETA SICH AUF DIE FALSCHEN FLAECHEN BEZIEHT
...
prob.blc = [2*sqrt(reshape(transpose(area),[],1)'), zeros(1, 2*n*m-n-m+2), -inf * ones(1, 2*m*n)];
prob.buc = [2*sqrt(reshape(transpose(area),[],1)'), zeros(1, 2*n*m-n-m), inf, inf, zeros(1, 2*m*n)];
...
% is stored' )
prob.bara.subi = [1:(m*n) sort([(n*m+1):(3*n*m-n-m) (n*m+1):(3*n*m-n-m)]) repmat(3*n*m-n-m+1, 1, n) repmat(3*n*m-n-m+2, 1, m) sort([(3*n*m-n-m+3):(5*n*m-n-m+2) (3*n*m-n-m+3):(5*n*m-n-m+2)])];
prob.bara.subj = subj; 
prob.bara.subk = [repmat(2, 1, n*m), ones(1,2*n*(m-1)), repmat(2, 1, 2*(n-1)*m) ones(1,n) 2*ones(1,m) repmat([2,1], 1, m*n) repmat([1,2], 1, m*n)];
prob.bara.subl = [ones(1,n*m), ones(1,2*n*(m-1)), repmat(2, 1, 2*(n-1)*m) ones(1,n) 2*ones(1,m) repmat([2,1], 1, m*n) repmat([1,2], 1, m*n)];
prob.bara.val  = [ones(1,n*m), repmat([1, -1], 1, n*(m-1)), repmat([1, -1], 1, (n-1)*m), -ones(1,n) -ones(1,m) repmat([eta, -1], 1, m*n) repmat([eta, -1], 1, m*n)];
