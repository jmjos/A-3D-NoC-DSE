...
%zuordnung zu den feldern wie im paper:
...
%% leicht modifizeirte loesungen

%zuordnung zu den feldern wie im paper:
testsolution = zeros(14,14);
testsolution(1,5) = 7+1;
testsolution(2,5) = 8+1;
testsolution(1,2) = 0+1;
testsolution(1,3) = 1+1;
testsolution(1,4) = 9+1;
testsolution(2,1) = 6+1;
testsolution(2,2) = 4+1;
testsolution(2,3) = 2+1;
testsolution(2,4) = 10+1;
testsolution(3,1) = 5+1;
testsolution(3,2) = 3+1;
testsolution(3,3) = 11+1;
testsolution(3,4) = 12+1;
testsolution(3,5) = 13+1;
...
limits = max([limits, sum(rows), sum(cols)]);
...
for i = 1:length(data)
    [x1, y1] = find(testsolution == data(i,1));
    [x2, y2] = find(testsolution == data(i,2));
    bandwidthModified = bandwidthModified + data(i, 3) * (abs(x1-x2) + abs(y1 - y2));
end
fprintf("communication modified  %5.2f\n", bandwidthModified);
fprintf("compared to paper %4.6f\n", bandwidthModified/bandwidthPaper);

% maximale bandbreite eines links nach routing algorithmus
bandwidthLinkMod = zeros(n-1, n-1);
for i = 1:length(data)
    [sourceX, sourceY] = find(testsolution == data(i,1));
    [dstX, dstY] = find(testsolution == data(i,2));
    B = sort([sourceX, dstX]);
    bandwidthLinkMod(B(1):B(2)-1, sourceY) = bandwidthLinkMod(B(1):B(2)-1, sourceY) + data(i,3);
    B = sort([sourceY, dstY]);
    bandwidthLinkMod(dstX, B(1):B(2)-1) = bandwidthLinkMod(dstX, B(1):B(2)-1) + data(i,3);
end
fprintf("max bandwidth modified  %5.2f\n", max(max(bandwidthLinkMod)));
fprintf("max bandwidth change  %5.5f\n", max(max(bandwidthLinkMod)) / max(max(bandwidthLinkPaper)));
...
%zuordnung zu den feldern wie im paper:
testsolution = zeros(14,14);
testsolution(1,1) = 5+1;
testsolution(1,2) = 13+1;
testsolution(1,3) = 4+1;
testsolution(1,4) = 11+1;
testsolution(2,1) = 6+1;
testsolution(2,2) = 3+1;
testsolution(2,3) = 0+1;
testsolution(2,4) = 12+1;
testsolution(3,1) = 1+1;
testsolution(3,2) = 2+1;
testsolution(3,3) = 7+1;
testsolution(4,1) = 8+1;
testsolution(4,2) = 9+1;
testsolution(4,3) = 10+1;
...
for i = 1:length(data)
    [x1, y1] = find(testsolution == data(i,1));
    [x2, y2] = find(testsolution == data(i,2));
    bandwidthDominik = bandwidthDominik + data(i, 3) * (abs(x1-x2) + abs(y1 - y2));
end
fprintf("communication modified  %5.2f\n", bandwidthDominik);
fprintf("compared to paper %4.6f\n", bandwidthDominik/bandwidthPaper);

% maximale bandbreite eines links nach routing algorithmus
bandwidthLinkDom = zeros(n-1, n-1);
for i = 1:length(data)
    [sourceX, sourceY] = find(testsolution == data(i,1));
    [dstX, dstY] = find(testsolution == data(i,2));
    B = sort([sourceX, dstX]);
    bandwidthLinkDom(B(1):B(2)-1, sourceY) = bandwidthLinkDom(B(1):B(2)-1, sourceY) + data(i,3);
    B = sort([sourceY, dstY]);
    bandwidthLinkDom(dstX, B(1):B(2)-1) = bandwidthLinkDom(dstX, B(1):B(2)-1) + data(i,3);
end
fprintf("max bandwidth modified  %5.2f\n", max(max(bandwidthLinkDom)));
fprintf("max bandwidth change  %5.5f\n", max(max(bandwidthLinkDom)) / max(max(bandwidthLinkPaper)));
